...
void reselect_example() {
  ...
  // auto memoize = flow::default_memoize<std::string, decltype(equality_checks), int, int>(equality_checks);  
  ...
  auto memoize = flow::map_memoize<std::string, decltype(keys), int, int>(keys);

  //
  auto show_multiply_selector = 
            flow::create_selector<State>(
...
void combine_selector(){
  ...
  auto keys = std::make_tuple(int_key, int_key);
  auto result_func_memoize = flow::map_memoize<std::string, decltype(keys), int, int>(keys);

  // Here is the first selector.
  auto string_concat_selector = 
            flow::create_selector<RootState>(
  ...
  auto another_result_func_memoize = flow::map_memoize<int, decltype(second_keys), std::string, int>(second_keys);

  // Here is the second selector
  auto combined_selector = 
          flow::create_selector<RootState>(
